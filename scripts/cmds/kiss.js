...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: "💋 Kiss generator",
    longDescription: "Create a romantic kiss image between two users",
    category: "love",
    guide: {
      en: "{pn} @user OR reply"
    }
  ...
  langs: {
    en: {
      ...
      fail: "❌ Failed to generate kiss image."
    }
  },
  ...
  onStart: async function ({ event, message, usersData, getLang }) {
    try {
      ...
      const [name1, name2] = await Promise.all([
        ...
        usersData.getName(uid2).catch(() => "Crush")
      ]);
      ...
      const apiURL = `${apiBase}/api/kiss?boy=${encodeURIComponent(
        avatar1
      ...
      const filePath = path.join(__dirname, "tmp", `kiss_${Date.now()}.jpg`);
      ...
      console.error("KISS CMD ERROR:", err);
